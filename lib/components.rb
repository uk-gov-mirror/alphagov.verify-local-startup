...
def list_components(repos)
  ...
  repos.each do |repo_name, config|
    ...
    case process["state"]
    when "running"      then running = "[ " + " Running ".green + " ]".ljust(10)
    when "missing"      then running = "[ " + "Not Built".red + " ]".ljust(10)
    else
      running = "[ " + " Stopped ".red + " ]".ljust(10)
    end
...
def rebild_component(repos, component_name)
...
def main()
  ...
  case args[:action]
    ...
    when "rebuild"  then rebild_component(repos, args[:component])
  else
